                                       ...
                                       unsigned module_index)
{
    if (module_index == -1 && symbols && symbols->pdbimport_size)
    {
        ...
        while (imp < (const PDB_SYMBOL_IMPORT*)last)
        {
            ...
            if (!_strnicmp(pdb_lookup->filename, imp->filename, strlen(imp->filename)))
            {
                             ...
                             struct pdb_lookup* pdb_lookup)
{
    ...
    if (ret)
    {
        ...
        utf8ztowchar(pdb_lookup->filename, msc_dbg->module->module.LoadedPdbName, ARRAY_SIZE(msc_dbg->module->module.LoadedPdbName));
